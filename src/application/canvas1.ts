...
export default class CanvasTrees {
  ...
  constructor(parentNode: HTMLElement) {
    //const raf = window.requestAnimationFrame(this.update);
    ...
    this.lights = new Lights(this.canvas.node);
...
class Lights {
  ...
  setLights() {
    ...
    this.ctx.setLineDash([2, 15])//, 5, 5]);
    this.ctx.lineCap = 'round';
    this.ctx.strokeStyle = '#09f'
    this.ctx.shadowBlur = 10;
    this.ctx.shadowColor = 'rgba(0.5, 0.35, 0.1, 0.5)';
    //this.ctx.beginPath();
    const radius = this.canvas.height * 0.7 / 5;
    const toRadians = Math.PI/180;
    for (let i = 1; i < 6; i++) {
      this.ctx.beginPath();
      this.ctx.arc(this.canvas.width / 2, 0.1 * this.canvas.height, radius * i, toRadians * 65, toRadians*115);
      this.ctx.stroke();
    }
   // this.ctx.stroke();
  }
